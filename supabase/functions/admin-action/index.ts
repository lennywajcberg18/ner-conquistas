...
Deno.serve(async (req) => {
  ...
  // ── APPROVE INVITE (creates user automatically) ──
  if (action === 'approve_invite') {
    ...
    if (!authError && authData.user) {
      ...
      await sendEmail(inv.email, '🎉 Bem-vindo ao Programa de Conquistas — Ner Israel!',
        `<p>Olá ${inv.name}!</p><p>Seu acesso foi aprovado!</p><p><strong>Email:</strong> ${inv.email}<br><strong>Senha temporária:</strong> ${tempPassword}</p><p>Acesse: <a href="${APP_URL}">${APP_URL}</a></p><p>Troque sua senha após o primeiro login.</p>`)
    }
    return new Response(JSON.stringify({ ok: true }), { headers: { ...cors, 'Content-Type': 'application/json' } })
  }
